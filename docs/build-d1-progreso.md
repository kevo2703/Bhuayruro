...
- [~] E12.5 Frontmatter regla 7 ✅ + URL entregada ✅. **PENDIENTE (requiere OK explícito de Kevin):** retirar `apps/admin` (el clasificador lo denegó por ser borrado de un dir no nombrado) + `git push origin d1-rebuild`. Credenciales dev en `apps/api/seeds/0001_seed_p0.sql` (seed sintético, nunca en chat).
...
- **Retirar `apps/admin` + `git push`**: el clasificador de auto-mode denegó `git rm -r apps/admin` (borrado de un dir pre-existente no nombrado por Kevin en esta sesión). Ambas acciones requieren **OK explícito de Kevin**. apps/admin es dead code (Next+Supabase, reemplazado por la SPA, D9); no lo referencia nada salvo un comentario en `packages/ui`.
